...
namespace GridGenerator
{
  ...
  //               // creating a 2d triangulation here with a single cell here
  //               with points located on the umin, umax, vmin, vmax boundaries
  ...
  //        ExcMessage("Something is odd with the topology of the CAD faces:
  //        number of vertices is not 4 times the number of faces"));

  //           // we know we'll have on cell per CAD surface
  //   cells.resize(face_count);

  //           //vertices are sorted cell by cell, so the connectivity is quite
  //           obvious
  ...
  //   triangulation.create_triangulation_compatibility(vertices, cells,
  //   subcelldata );
  ...
  template void
    subdivided_hyper_rectangle<1>(Triangulation<1> &,
                                  const std::vector<std::vector<double>> &);
  template void
    subdivided_hyper_rectangle<2>(Triangulation<2> &,
                                  const std::vector<std::vector<double>> &);
  template void
    subdivided_hyper_rectangle<3>(Triangulation<3> &,
                                  const std::vector<std::vector<double>> &);
  template void
    subdivided_hyper_rectangle<1, 2>(Triangulation<1, 2> &,
                                     const std::vector<std::vector<double>> &);
  template void
    subdivided_hyper_rectangle<1, 3>(Triangulation<1, 3> &,
                                     const std::vector<std::vector<double>> &);
  template void
    subdivided_hyper_rectangle<2, 3>(Triangulation<2, 3> &,
                                     const std::vector<std::vector<double>> &);
} // namespace GridGenerator
