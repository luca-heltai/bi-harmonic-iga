...
FullMatrix<double>
kronecker_product(const FullMatrix<double> &a, const FullMatrix<double> &b)
{
...
void
reduced_row_echelon_form(FullMatrix<double> &       matrix,
...
IgaHandler<dim, spacedim>::IgaHandler(
  ...
  , local_quad(QIterated<dim>(QTrapez<1>(), 30))
{
  ...
  map_fe = std::make_unique<MappingFEField<dim, spacedim>>(dh_sys, euler, mask);

  std::vector<std::vector<double>> knots_with_repetition(knot_vectors);
  for (int i = 0; i < dim; ++i)
    {
      for (int j = mults[i].size() - 1; j >= 0; --j)
        knots_with_repetition[i].insert(knots_with_repetition[i].begin() + j,
                                        mults[i][j] - 1,
  ...
  for (typename DoFHandler<dim, spacedim>::active_cell_iterator cell =
       ...
       ++cell, ++el)
    {
      ...
      for (unsigned int d = 0; d < dim; ++d)
        {
          extractors[d].local_b_extractor =
            local_b_extractors[d][cell_coordinates[d]];
      ...
      iga_objects[cell] = ex;
      std::vector<std::size_t> col_order_t(col_order.begin(), col_order.end());
      std::vector<std::size_t> row_order_t(row_order.begin(), row_order.end());

      iga_objects[cell].local_b_extractor.fill_permutation(ex.local_b_extractor,
                                                           row_order_t,
                                                           col_order_t);
    }
...
template <int dim, int spacedim>
unsigned int
IgaHandler<dim, spacedim>::n_cells(
...
template <int dim, int spacedim>
unsigned int
IgaHandler<dim, spacedim>::get_first_basis(
  const std::vector<unsigned int> &mult,
...
template <int dim, int spacedim>
void
IgaHandler<dim, spacedim>::assemble_global_extractor()
{
...
template <int dim, int spacedim>
void
IgaHandler<dim, spacedim>::output_basis()
{
...
IgaHandler<dim, spacedim>::get_global_extractor() const
{
...
template <int dim, int spacedim>
const std::map<typename DoFHandler<dim, spacedim>::active_cell_iterator,
               BezierExtractor<dim>> &
IgaHandler<dim, spacedim>::get_iga_objects() const
{
  return iga_objects;
}
  ...
  typename DoFHandler<dim>::active_cell_iterator &cell)
{
  ...
  FullMatrix<double> cell_bspline_matrix_buffer(fe.dofs_per_cell,
                                                fe.dofs_per_cell);

  src.mTmult(cell_bspline_matrix_buffer, iga_objects[cell].local_b_extractor);

  iga_objects[cell].local_b_extractor.mmult(dst, cell_bspline_matrix_buffer);
}
...
IgaHandler<dim, spacedim>::project_boundary_values(
  std::map<types::boundary_id, const Function<spacedim> *> &boundary_function,
  ...
  AffineConstraints<double> &                               constraints)
{
  ...
  std::map<types::global_dof_index, double>::iterator fake_it =
    fake_boundary_values.begin();
  ...
  std::map<types::global_dof_index, double>::iterator it =
  ...
  std::vector<Point<dim>> &support_points)
{
  ...
  std::vector<std::vector<unsigned int>> mapp(
    dim, std::vector<unsigned int>(dh.n_dofs()));

  if (fe.degree > 2)
    {
      // we need to reorder, in order to find correctly the degrees of
      // freedom of the system dof handler.
      FESystem<dim, spacedim>   feqs(FE_Q<dim, spacedim>(fe.degree), spacedim);
      ...
      typename DoFHandler<dim, spacedim>::active_cell_iterator
        ...
        cellqs = dhqs.begin_active(), endc = dhq.end();
          ...
          for (unsigned int i = 0; i < feqs.dofs_per_cell; ++i)
            for (unsigned int k = 0; k < feq.dofs_per_cell; ++k)
              if (sp_qs[i].distance(sp_q[k]) < 1e-6)
                mapp[feqs.system_to_component_index(i).first][dofs_q[k]] =
                  dofs_qs[i];
        }
    }
  else
  ...
  std::vector<Vector<double>> bspline_euler(dim, Vector<double>(n_bspline));

  std::vector<Vector<double>> euler_comp(dim, Vector<double>(dh.n_dofs()));
  ...
  for (unsigned int d = 0; d < dim; ++d)
    {
      ...
      transform_vector_into_bspline_space(bspline_euler[d], euler_comp[d]);
  ...
  AffineConstraints<double> &                     constraints)
{
  ...
  transform_cell_rhs_into_bspline_space(bspline_cell_rhs, cell_rhs, cell);
...
IgaHandler<dim, spacedim>::assemble_interpolation_matrices()
{
  ...
  for (typename DoFHandler<dim, spacedim>::active_cell_iterator cell =
       ...
       ++cell, ++el)
    {
      ...
      cell->get_dof_indices(dofs);
      std::vector<unsigned int> &bspline_dofs = iga_objects[cell].dof_indices;
      ...
      for (unsigned int q = 0; q < local_quad.size(); ++q)
        {
          for (unsigned int j = 0; j < fe.dofs_per_cell; ++j)
            {
              for (unsigned int i = 0; i < fe.dofs_per_cell; ++i)
                BsplineInterpolation(el * local_quad.size() + q,
                                     bspline_dofs[j]) +=
  ...
  const std::vector<double> &      knot_vector)
{
  ...
  std::set<double> elements_edges_set(knot_vector.begin(), knot_vector.end());
      ...
      if (mult[element + 1] < degree)
        {
          ...
          for (unsigned int j = degree; j > mult[element + 1]; --j)
            alphas[j - mult[element + 1] - 1] =
              numer / (knot_vector[a + j] - knot_vector[a]);
              ...
              for (unsigned int k = degree + 1; k > s; --k)
                {
                  ...
                  for (unsigned int row = 0; row < degree + 1; ++row)
                    local_extractors[element][row][k - 1] =
                      alpha * local_extractors[element][row][k - 1] +
...
template class IgaHandler<1, 1>;
// template class IgaHandler<1, 2>;
// template class IgaHandler<1, 3>;

template class IgaHandler<2, 2>;
// template class IgaHandler<2, 3>;

template class IgaHandler<3, 3>;
