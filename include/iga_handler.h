...
#ifndef dealii_iga_handler_h
#define dealii_iga_handler_h
...
/*
 * @author Marco Tezzele, Luca Heltai, 2013, 2019
 */
...
struct UnivariateBezierExtractor
{
  ...
  UnivariateBezierExtractor(unsigned int degree, unsigned int n_dofs)
    : degree(degree)
...
// This function performs a Kronecker product between two full matrices given as
// input and returns the resulting full matrix
FullMatrix<double>
kronecker_product(const FullMatrix<double> &a, const FullMatrix<double> &b);

// Misleading name. This function is needed to reorder the dofs when the
// kronecker product between extractors is performed. Nb is the number of
// degrees of freedom. a and b are the vectors of the local dofs of the
// UnivariateBezierExtractor
...
struct BezierExtractor
{
    ...
    , n_dofs(1)
  {
    ...
    for (unsigned int i = 1; i < dim; ++i)
      {
        local_b_extractor =
          kronecker_product(extractors[i].local_b_extractor, local_b_extractor);
        dof_indices =
          kronecker_product(extractors[i].dof_indices, dof_indices, n_dofs);
...
public:
  ...
  unsigned int
  n_cells(const std::vector<std::vector<double>> &p) const;

  // Returns the first non zero basis function with support on cell $el
  // on one direction
  unsigned int
  get_first_basis(const std::vector<unsigned int> &mult,
  ...
  void
  assemble_interpolation_matrices();
  ...
  // dts = (G * G^T)^-1 * G * src
  void
  transform_vector_into_bspline_space(Vector<double> &      dst,
                                      const Vector<double> &src) const;

  // This function transforms a cell rhs from the FE space into the bspline
  // space. This is done using the LocalExtractor C: dst = C * src
  void
  transform_cell_rhs_into_bspline_space(
    Vector<double> &                                dst,
    Vector<double> &                                src,
    typename DoFHandler<dim>::active_cell_iterator &cell);

  // This function transform a cell matrix from the FE space into the bspline
  // space. This is done using the LocalExtractor C: dst = C * src * C^T
  void
  transform_cell_matrix_into_bspline_space(
  ...
  void
  map_dofs_to_support_points(std::vector<Point<dim>> &support_points);
  ...
  void
  make_sparsity_pattern(DynamicSparsityPattern &dsp);
  ...
  Vector<double>                                 euler;
  std::unique_ptr<MappingFEField<dim, spacedim>> map_fe;
  ...
  void
  compute_local_b_extractors(std::vector<FullMatrix<double>> &local_extractors,
